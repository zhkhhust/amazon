...
function getTogetherProducts($pageSource) {
...
function getTogetherProducts2($pageSource) {
    $datas = array();
    $recommendations = array();
    ...
    //$count = count($recommendations[1]);
    for ($idx = 0; $idx < $count; ++$idx) {
        $datas[] = array($titles[1][$idx], $url_imgs[1][$idx], $url_imgs[2][$idx], $stars[1][$idx],
            $views[1][$idx], $prices[1][$idx]);
    }
    return $datas;
}

function getTogetherProducts3($pageSource) {
    $datas = array();
    $recommendations = array();
    ...
    //$count = count($recommendations[1]);
    for ($idx = 0; $idx < $count; ++$idx) {
        $datas[] = array($url_titles[1][$idx], $imgs[1][$idx], $url_titles[2][$idx], $stars[1][$idx],
            $views[1][$idx], $prices[1][$idx]);
    }
    return $datas;
}
...
function processProductPage($pageSource, $id, $cat_id) {
    ...
    $product = array(
        ...
        'goods_img' => '', 'site_url' => '', 'title' => $title, 'path' => $path, 'data_added' => time()
    );
    echo insert_product($product);
    //print_r($product);
    /*
      "insert into goods(`cat_id`, `goods_sn` , `goods_name`, `goods_name_style`, `click_count`, `brand_id`, `provider_name`,
      `goods_number`, `goods_weight`, `market_price`, `shop_price`, `promote_price`, `keywords`,`goods_desc`,`goods_thumb`,
      `goods_img`, `site_url`, `title`, `path`) values($cat_id, '$goodsSn', '$dimension', '$viewer', 100, '$productWeight',
      '', '$productWeight', '$maket_price', '$sell_price', '', '$keywords', '$desc', '$path',
      '$img', '$url', */
//        echo getProductMarketDate($pageSource);
//        echo $price;
    /*
      $addTime = dateToInt();
      echo $addTime; */
 ...
 */
$sql = "select id, url, cat_id from dm_product where grabbed!=1 limit 200";
